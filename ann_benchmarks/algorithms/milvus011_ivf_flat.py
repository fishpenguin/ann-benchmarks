...
class MilvusIVFFLAT(BaseANN):
    ...
    def already_fit(self, total_num):
        ...
        has_table = self._milvus.has_collection(self._table_name)
        if has_table:
            table_stats = self._milvus.get_collection_stats(self._table_name)
            if table_stats:
...
#             self._milvus_id_to_index[id] = i
    ...
    def query(self, v, n):
        ...
        future = self._milvus.search(self._table_name, dsl, _async=True)
        return future
    ...
    def batch_query(self, X, n):
        ...
        results = self._milvus.search(self._table_name, dsl)
        # if not status.OK():
        #     raise Exception("[Search] search failed: {}".format(status.message))
