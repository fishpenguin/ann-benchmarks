...
class MilvusIVFFLAT(BaseANN):
    def __init__(self, metric, dataset, index_type, nlist):
        ...
        self._milvus.connect(host='localhost', port='19537')
        # import uuid
    ...
    def already_fit(self, total_num):
        ...
        if has_table:
            status, table_stats = self._milvus.collection_info(self._table_name)
            if table_stats:
                row_nums = table_stats['row_count']
                if row_nums >= total_num:
                    # self._already_nums == row_nums
                    status, index_info = self._milvus.describe_index(self._table_name)
